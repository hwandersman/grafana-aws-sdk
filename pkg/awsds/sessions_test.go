...
type mockStsCredentialsGetter struct {
	mock.Mock
}

func (m *mockStsCredentialsGetter) NewCredentials(c client.ConfigProvider, roleARN string,
	options ...func(*stscreds.AssumeRoleProvider)) *credentials.Credentials {
	args := m.Called(c, roleARN, options)

	return args.Get(0).(*credentials.Credentials)
}
...
func TestNewSession_GrafanaAssumeRole(t *testing.T) {
	...
	t.Run("externalID is passed to the session", func(t *testing.T) {
		stsCredentialsGetter := &mockStsCredentialsGetter{}
		stsCredentialsGetter.On("NewCredentials", mock.Anything, mock.Anything,
			mock.MatchedBy(func(options []func(*stscreds.AssumeRoleProvider)) bool {
				p := &stscreds.AssumeRoleProvider{RoleARN: "test_arn"}
				require.Len(t, options, 1)
				options[0](p)
				require.NotNil(t, p)
				assert.Equal(t, "grafanamagic", *p.ExternalID)
				return *p.ExternalID == "grafanamagic"
			})).Return(&credentials.Credentials{})

		require.NoError(t, os.Setenv(AllowedAuthProvidersEnvVarKeyName, "grafana_assume_role"))
		require.NoError(t, os.Setenv(AssumeRoleEnabledEnvVarKeyName, "true"))

		cache := NewSessionCache(stsCredentialsGetter)
